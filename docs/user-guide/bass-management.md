...
>
> The bass manager directs bass frequencies from *all* channels to one or more subwoofers, not just the content of the LFE channel. However, when there is no subwoofer, the bass manager would direct the LFE channel to the main speakers. This is the only time the LFE channel would *not* be sent to the subwoofer.
>
> The key concept is that the LFE channel is not the "subwoofer channel".

<!-- verify: whether the HTP-1's own behavior with zero subwoofers configured matches this Wikipedia description exactly -->
...
- A speaker set to **Small** (or **Dolby**) gets a crossover frequency field, range 40–200 Hz in steps of 10, and its bass below that frequency is redirected to the subwoofer(s).
- A speaker set to **Large** is treated as full-range and keeps its own bass.
- Subwoofers have no size or crossover control of their own — the **LPF for LFE Channel** control above governs the low end sent to them instead.

!!! tip
    If you don't have a spec sheet for your speakers, a basic Dirac Live calibration will show you the speaker's useful range. You can use that measurement to choose a crossover frequency. Unless you really have "Large" full-range speakers, set the size to "Small" or "Dolby" and pick a cutoff within the effective range of the speaker.
...
A tactile "seat shaker" output can be assigned to one of the subwoofer channels, giving low-frequency effects a physical, felt presence in the seating area in addition to what's heard. The Seat Shaker page (sidebar) has its own controls: content source (Downmix, LFE Only, or Auto), a low-pass filter, delay, LFE gain, trim, and a 4/8/16-band PEQ.

The seat shaker channel consumes one of the 16 available output channels. It appears in the speaker layout table with a couch icon instead of a checkbox, and is highlighted yellow (not green) on the Speaker Map. It is excluded from Dirac Live calibrations and receives no Dirac Live filter correction while Dirac Live is enabled.

!!! note
    Bass EQ (`/#/settings/bass-eq`, sidebar entry **Bass EQ**) is a separate feature: it pulls filters from a catalog to correct specific known room or speaker bass anomalies. It is not the same thing as bass management, and does not appear on the Speakers page.
